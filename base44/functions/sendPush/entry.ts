...
Deno.serve(async (req) => {
  try {
    ...
    // 🔧 TEMPLATE: OneSignal credentials — set the ONESIGNAL_APP_ID and
    // ONESIGNAL_REST_API_KEY secrets (dashboard → settings → environment variables),
    // or replace the fallback strings below. Both come from OneSignal → Settings → Keys & IDs.
    const oneSignalAppId = Deno.env.get('ONESIGNAL_APP_ID') || 'YOUR_ONESIGNAL_APP_ID';
    const oneSignalRestKey = Deno.env.get('ONESIGNAL_REST_API_KEY') || 'YOUR_ONESIGNAL_REST_API_KEY';
    ...
    if (oneSignalAppId.startsWith('YOUR_')) {
